...
import { CAlert, CBadge, CButton, CCard, CCardBody, CCardHeader, CCardText, CCol, CForm, CFormLabel, CFormSelect, CListGroup, CListGroupItem, CModal, CModalBody, CModalFooter, CModalHeader, CModalTitle, CRow, CSpinner } from '@coreui/react'
import { useDispatch, useSelector } from 'react-redux'
import { clearErrors, allOrders, getOrderDetails, processOrder } from 'src/actions/orderActions'
import CIcon from '@coreui/icons-react'
import { UPDATE_ORDER_RESET } from 'src/constants/orderConstants'
...
const ProcessOrder = ({ match }) => {
    ...
    const { error: updateError, isUpdated, loading: loadingOrder } = useSelector(state => state.order);
    ...
    useEffect(() => {
        if (order) {
            ...
            setRows(order?.orderItems?.map(item => ({
                productID: item.productID,
                name: item.name,
                price: item.price,
                image: <img src={item.image} alt="" width="100" height="100" style={{objectFit: "contain"}}></img>,
              ...
              <CRow>
                <CForm onSubmit={handleSubmit}>
                  ...
                  <CCol xs="8">
                    ...
                    <CFormSelect value={status} onChange={(e) => setStatus(e.target.value)} id="category">
                      <option disabled={true} value="Processing">Processing</option>
                      <option disabled={status !== "Processing"} value="In Transit">In Transit</option>
                      <option disabled={status !== "In Transit"} value="Delivered">Delivered</option>
                    </CFormSelect>
                  </CCol>
                  <CCol xs="4">
                    <CButton disabled={loadingOrder || loading} type="submit">
                      {!loading && !loadingOrder ? "Save" : (
                          <>
                      ...
                      <CCardText>
                        ...
                        <CListGroup>
                          <CListGroupItem className="d-flex justify-content-between align-items-center">
                            ...
                            <CBadge color={
                              order.paymentInfo?.status === "succeeded" ? "success"
                              : "warning"
                            }>
                              {order.paymentInfo?.status === "succeeded" ? <h5>PAID</h5> : <h5>NOT PAID</h5>}
                            </CBadge>
                          </CListGroupItem>
                        </CListGroup>
                        <CListGroup>
                        ...
                        <CListGroup>
                          {order.deliveredOn ?
                          <CListGroupItem className="d-flex justify-content-between align-items-center">
                            <p><strong>Delivery Confirmed On</strong></p>
                          ...
                          :<CListGroupItem className="d-flex justify-content-between align-items-center">
                            ...
                            <p>{order.createdOn?.toString().slice(0, 10)}</p>
                          </CListGroupItem>}
                        </CListGroup>
                      </CCardText>
                    </CCardBody>
